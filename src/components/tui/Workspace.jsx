...
const Workspace = ({
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === 'ArrowDown') { e.preventDefault(); const ni = idx < 0 ? 0 : (idx + 1) % items.length; const n = items[ni]; if (n) setSelected(n); }
      if (e.key === 'ArrowUp')   { e.preventDefault(); const ni = idx < 0 ? items.length - 1 : (idx - 1 + items.length) % items.length; const n = items[ni]; if (n) setSelected(n); }
  ...
  const Explorer = (
    <div className={`p-2 overflow-y-auto ${ACCENT_DIV[accent] || ACCENT_DIV.green}`} aria-label="File explorer">
  ...
  return (
    <div className={`border ${ACCENT_BORDER[accent] || ACCENT_BORDER.green} rounded-md font-mono bg-black/40`}>
      {/* Path bar */}
      <div className={`flex items-center justify-between px-3 py-2 border-b ${ACCENT_DIV[accent] || ACCENT_DIV.green} text-xs`}>
        <div className="flex items-center gap-2 min-w-0">
          <button type="button" className="md:hidden text-white/60" onClick={() => setDrawerOpen((v) => !v)} aria-label="Toggle explorer"><Menu size={16} /></button>
        ...
        <div className="flex items-center gap-2">
          {selected && (
            <>
              <button type="button" onClick={() => setFullscreen((v) => !v)} className="p-1.5 border border-white/20 rounded hover:border-white/40" title="Fullscreen (f)">
                {fullscreen ? <Minimize2 size={14} /> : <Maximize2 size={14} />}
              </button>
              <button type="button" onClick={() => window.open(selected.file, '_blank', 'noopener')} className={`p-1.5 border border-white/20 rounded ${colors.hoverBorder} ${colors.textHover}`} title="Open in new tab">
      ...
      {loading ? (
        ...
        <div className={`grid ${fullscreen ? 'grid-cols-1' : 'md:grid-cols-[260px_1fr]'} min-h-[60vh]`}>
          ...
          {!fullscreen && (
            <div className={`md:border-r ${ACCENT_DIV[accent] || ACCENT_DIV.green} ${drawerOpen ? 'block' : 'hidden md:block'}`}>{Explorer}</div>
          )}
          {/* Viewer */}
          <div className="flex flex-col">
            <div className={`flex items-center gap-2 px-3 py-2 border-b ${ACCENT_DIV[accent] || ACCENT_DIV.green} text-xs`}>
